# 作者：算个文科生吧（ljh_G335 补丁）
# 联系方式：lijinghailjh@163.com
# 作用：Gemini 330 系列驱动启动；本仓库增强 YAML 合并、bringup 配置路径、自动 RViz
import os
import sys
import yaml
...
from utils_yaml import load_and_parse_yaml, get_absolute_path, update_params  # noqa: E402


def resolve_config_path(config_file_path: str) -> str:
    if not config_file_path:
        return ''
    if os.path.isabs(config_file_path) and os.path.isfile(config_file_path):
        return config_file_path
    if os.path.isfile(config_file_path):
        return os.path.abspath(config_file_path)
    try:
        bringup = get_package_share_directory('ljh_g335_bringup')
        c = os.path.join(bringup, 'config', os.path.basename(config_file_path))
        if os.path.isfile(c):
            return c
    except Exception:
        pass
    return get_absolute_path(config_file_path, 'orbbec_camera')
...
def load_parameters(context, args):
    ...
    config_file_path = resolve_config_path(args_params.get('config_file_path', ''))
    if config_file_path:
        yaml_params = load_and_parse_yaml(config_file_path)
    else:
        yaml_params = update_params(
            load_and_parse_yaml(get_absolute_path('common.yaml', 'orbbec_camera')),
            load_and_parse_yaml(get_absolute_path('gemini330_series.yaml', 'orbbec_camera')),
        )
    default_params = update_params(args_params, yaml_params)
    cmd = dict([x for x in [a.split(':=') for a in sys.argv] if len(x) == 2])
    default_params = update_params(default_params, cmd)
    if args_params.get('serial_number'):
        default_params['serial_number'] = args_params['serial_number']
    if args_params.get('usb_port'):
        default_params['usb_port'] = args_params['usb_port']
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument('rviz_delay', default_value='2.5'),
        #infra1
    ...
    def launch_setup(context, launch_args):
        actions = create_node_action(context, launch_args)
        rviz_cfg = LaunchConfiguration('rviz_config_file').perform(context)
        if not rviz_cfg:
            rviz_cfg = os.path.join(
                get_package_share_directory('orbbec_camera'), 'rviz', 'gemini_330_series.rviz')
    ...
    return LaunchDescription(
        args + [OpaqueFunction(function=lambda context: launch_setup(context, args))]
    )
